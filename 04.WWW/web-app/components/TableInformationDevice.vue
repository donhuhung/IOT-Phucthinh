<template>
  <div>
    <h2 class="m-0 px-4 py-2 text-sm bg-blue-200 text-blue-600">
      {{ title }}
    </h2>
      ...
      <table class="min-w-max w-full table-auto">
        ...
        <tr class="text-gray-600 border-b border-gray-200 uppercase text-sm leading-normal">
          <template v-for="field in headers">
            <th class="py-3 px-6 text-left" :key="field.name">
              {{ field.label }}
            </th>
          </template>
        </tr>
        ...
        <template v-for="(row, index) in items">
          <tr :key="index" class="border-b border-gray-200 hover:bg-gray-100 row-item">
            <template v-for="field in headers">
              <td :key="`${row.id}-${field.name}`" class="px-6 text-left whitespace-nowrap">
    ...
    rows() {
      ...
      /*return Array.from({length: 2}).map(item => {
        ...
        headers.forEach(header => {
          field[header.name] = faker.name.findName()
